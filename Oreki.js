        ...
        const message = {
            ...
            contextInfo: {
                ...
                externalAdReply: {
                    ...
                    renderLargerThumbnail: false // Set to false for a smaller render
                }
            }
        };
...
if (m.mtype === 'locationMessage' || m.mtype === 'liveLocationMessage') {
 ...
 const fvideo = {key: { fromMe: false,participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: "status@broadcast" } : {}) },message: { "videoMessage": { "title":"ᎧᏒᏋᏦᎥ Ꮙ3 🀄⛩️👘︎", "h": "༆༊ 𝐓𝕳𝖀𝕲 ᵒᵗᶠ ᝰ.ᐟ︎",'seconds': '359996400', 'caption': `${pushname}`, 'jpegThumbnail': thumb}}}
...
function getRandomSymbol() {
    const symbols = ['⋆𐙚₊˚⊹♡', '⋆˚𝜗𝜚˚⋆', '⍣ ೋ︎', 'ᝰ.ᐟ︎','𝓙ᥫ᭡','༆','°❀⋆.ೃ࿔*:･︎','༊*·˚︎','*ೃ༄︎']; // Add more symbols as needed
...
case 'alive':
    ...
    const dooc = {
        ...
        contextInfo: {
            ...
            externalAdReply: {
                title: "ᎧᏒᏋᏦᎥ Ꮙ3 🀄⛩️👘", // Title for the message
                body: "© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝚃𝙷𝚄𝙶  𝚃𝙴𝙲𝙷𝙸𝙴𝚂™ 🐣", // Body text for the message
...
case 'runtime':
    if (isBan) return reply(mess.banned);
                                 let runtimetext = `*Service has remained uninterrupted for ${runtime(process.uptime())} 🐎*`;
                                        ...
                                        m.chat,
                                        {
                                            ...
                                            contextInfo: {
                                                externalAdReply: {
                                                    showAdAttribution: true,
                                                    title: botname,
                                                    body: `© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝚃𝙷𝚄𝙶  𝚃𝙴𝙲𝙷𝙸𝙴𝚂™ 🐣`,
                                                    thumbnailUrl:
                                                    "https://i.imgur.com/HywKbSx.jpeg",
                                                    sourceUrl: "https://whatsapp.com/channel/0029VaoOiuwDp2QH070eTE01",
                                                    mediaType: 1,
...
case 'play': {
    ...
    try {
        ...
        if (data.status !== 200 || !data.success || !data.result?.download_url) {
            return reply(`*❌ Failed to download the audio. Please try again later.*`);
        }
        ...
        reply(`*❌ Error occurred!*\n_It could be a connection or API issue._`);
    }
...
if (!isPremium) return replyprem(mess.premium)
    ...
    try {
        let { data } = await axios.get(`https://apis.davidcyriltech.my.id/search/playstore?q=${encodeURIComponent(text)}`);

        if (!data.success || !data.result) return reply("❌ No results found. Please check the app name and try again.");

        let app = data.result;

        let message = `🔍 *Play Store Search Result:*\n\n`;
        message += `📌 *Title:* ${app.title}\n`;
        message += `📄 *Summary:* ${app.summary}\n`;
        message += `📥 *Installs:* ${app.installs}\n`;
        message += `⭐ *Rating:* ${app.score}\n`;
        message += `💲 *Price:* ${app.price}\n`;
        message += `📏 *Size:* ${app.size}\n`;
        message += `📱 *Android Version:* ${app.androidVersion}\n`;
        message += `👨‍💻 *Developer:* ${app.developer}\n`;
        message += `📆 *Released:* ${app.released}\n`;
        message += `🛠️ *Updated:* ${app.updated}\n`;
        message += `🆕 *Latest Version:* ${app.version}\n\n`;
        message += `🔗 *View on Play Store:* ${app.url}\n\n`;

        // Send app icon
        await sendImage(app.icon, "🖼️ *App Icon*");

        // Send up to 3 screenshots
        if (app.screenshots && app.screenshots.length > 0) {
            for (let i = 0; i < Math.min(app.screenshots.length, 3); i++) {
                await sendImage(app.screenshots[i], `🖼️ *Screenshot ${i + 1}*`);
            }
        }
        ...
        console.error(error);
        reply("*⚠️ Error fetching Play Store data. Please try again later.*");
    }
    break;
...
case 'autostatusreact': {
    if (isBan) return reply(mess.banned);
                ...
                if (args[0] === 'on') {
                    ...
                    reply(`Bot Autostatus React is enabled. Bot will react to statuses with "${reactEmoji}".`);
                } else if (args[0] === 'off') {
                ...
                if (autostatusReact) {
                    try {
                        const statusList = await ThugBotInc.fetchStatusUpdates(); // Fetch all status updates
                        for (const status of statusList) {
                            await ThugBotInc.sendMessage(status.sender, {
                                react: {
                        ...
                        reply('Failed to react to statuses.');
                    }
                }
                break;
...
case 'remini': {
    if (!quoted) return reply("*Where is the image?*\n*Kindly provide an image.*");
    if (!/image/.test(mime)) return reply(`*Send/Reply Photos With Captions ${prefix + command}*`);
    
      // Ensure this function properly handles loading
...
case 'removebg': {
    ...
    try {
        ...
        const res = await fetch(`https://apis.davidcyriltech.my.id/removebg?url=${encodeURIComponent(mediaUrl)}`);
        const buffer = await res.buffer(); // Get the response as a binary buffer

        // Check if the response is valid and is an image buffer
        if (!buffer) {
            throw new Error("Background removal failed. No image data returned.");
        }

        // Send the processed image (background removed) with quoted message
        await ThugBotInc.sendMessage(m.chat, {
            image: buffer,
            caption: `*Background Removed Successfully*\n*© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝚃𝙷𝚄𝙶  𝚃𝙴𝙲𝙲𝙷𝙸𝙴𝚂™ 🐣*`,
            quoted: m  // Ensure the message is quoted
        });
...
case "menu":
    if (isBan) return reply(mess.banned);
    try {
        let greetingText = `*👋 𝙷𝙴𝙻𝙻𝙾, ${pushname}. 𝙸’𝙼 ᎧᏒᏋᏦᎥ Ꮙ3 🀄⛩️👘, 𝚈𝙾𝚄𝚁 𝙿𝙴𝚁𝚂𝙾𝙽𝙰𝙻 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿 𝙰𝚂𝚂𝙸𝚂𝚃𝙰𝙽𝚃 — 𝙷𝙴𝚁𝙴 𝚃𝙾 𝙼𝙰𝙺𝙴 𝚈𝙾𝚄𝚁 𝚃𝙰𝚂𝙺𝚂 𝚂𝙼𝙾𝙾𝚃𝙷𝙴𝚁 𝙰𝙽𝙳 𝙴𝙵𝙵𝙸𝙲𝙸𝙴𝙽𝚃. 𝙹𝚄𝚂𝚃 𝚂𝙰𝚈 𝚃𝙷𝙴 𝚆𝙾𝚁𝙳.*\n`;
        greetingText += `*💡 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙷𝙸𝙽𝚃: 𝚄𝚂𝙴 ${prefix}𝙼𝙴𝙽𝚄𝙻𝙸𝚂𝚃 𝚃𝙾 𝚅𝙸𝙴𝚆 𝙰𝙻𝙻 𝙰𝚅𝙰𝙸𝙻𝙰𝙱𝙻𝙴 𝙾𝙿𝚃𝙸𝙾𝙽𝚂.*`;

        const buttons = [
            { buttonId: "menulist", buttonText: { displayText: "📜 𝚅𝙸𝙴𝚆 𝙼𝙴𝙽𝚄" }, type: 1 }
        ];

        await ThugBotInc.sendMessage(m.chat, {
            video: { url: "https://files.catbox.moe/jsekqi.mp4" },
                ...
                for (let i of anu) {
                    ...
                    ThugBotInc.sendMessage(i, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                showAdAttribution: true,
                                title: "© 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝚃𝙷𝚄𝙶  𝚃𝙴𝙲𝙷𝙸𝙴𝚂™ 🐣",
                                body: `Sent in ${i.length} Group`,
                                thumbnailUrl: 'https://files.catbox.moe/im744a.jpg',
                                sourceUrl: 'https://whatsapp.com/channel/0029VaoOiuwDp2QH070eTE01',
                                mediaType: 1,
                                renderLargerThumbnail: true
                            }
...
╰──❀༻❁༺༻❁༺༻❁༺❀─╯`;
        ...
        const imageUrls = [
            "https://files.catbox.moe/uk2f7y.jpg",
            "https://files.catbox.moe/81nlcb.jpg",
            "https://files.catbox.moe/bxeoe4.jpg",
            "https://files.catbox.moe/3deduz.jpg",
            "https://files.catbox.moe/5j6tgv.jpg",
            "https://files.catbox.moe/328fcc.jpg",
            "https://files.catbox.moe/ujb6ql.jpg",
            "https://files.catbox.moe/5akon2.jpg",
            "https://files.catbox.moe/a7a3of.jpg",
            "https://files.catbox.moe/mynzw0.jpg",
            "https://files.catbox.moe/araaz0.jpg"
        ];
        ...
        await ThugBotInc.sendMessage(m.chat, {
            ...
            contextInfo: {
                ...
                externalAdReply: {
                    ...
                    sourceUrl: "https://whatsapp.com/channel/0029VaoOiuwDp2QH070eTE01", // optional
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
            }
        ...
        await ThugBotInc.sendMessage(m.chat, {
            audio: { url: "https://files.catbox.moe/qz42l6.m4a" },
            mimetype: "audio/mpeg",
            ptt: true
        }, { quoted: m });
...
ThugBotInc.sendMessage(m.chat, {image: {url: avatar }, caption: `> © 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝚃𝙷𝚄𝙶  𝚃𝙴𝙲𝙷𝙸𝙴𝚂™ 🐣` }, {quoted: m })
}
break
...
case 'tomp3': {
    ...
    return reply(`*Please send or reply to a video/audio file that you wish to convert to MP3, along with the caption ${prefix + command}*`);
                let media = await ThugBotInc.downloadMediaMessage(qmsg)
                let audio = await toAudio(media, 'mp4')
                ThugBotInc.sendMessage(m.chat, {
                    document: audio,
                    mimetype: 'audio/mp3',
                    fileName: `ᎧᏒᏋᏦᎥ Ꮙ3 🀄⛩️👘.mp3`
                }, {
                    quoted: m
                })
...
if (isBan) return reply(mess.banned);
    ...
    if (!isBotAdmins) return reply(mess.botadmin); // Ensure bot has admin privileges
...
case 'tosketch':
    ...
    try {
        ...
        if (!data) {
            return reply("*Error: Unable to download the image.*");
        }

        const media = await ThugBotInc.downloadAndSaveMediaMessage(q);
        if (!media) {
            return reply("*Error: Unable to process the image.*");
        }

        const mediaUrl = await handleMediaUpload(m, mime);
        if (!mediaUrl || !mediaUrl.startsWith("http")) {
            return reply("*Error: Failed to upload the image.*");
        }
        ...
        console.error('Error in tosketch command:', error);
        reply(`*AN ERROR OCCURRED!!*\n\n> ${error.message}`);
    }
...
case 'repo':
case 'botrepo': {
    let runtimet = `*Hey 👋 ${pushname}, kindly support my project. 🌼*\nhttps://github.com/OTAKUSYNTAX/OREKI_V3`;
...
case 'update': {
  ...
  const DB = require('./lib/update');
  try {
    const commits = await DB.syncgit();
    if (commits.total === 0) {
      reply(`✅ *${m.pushName}*, you already have the latest version.`);
    } else {
     reply(`🚀 *${commits.total} new updates found!*\nStarting update...`);
      const updateResult = await DB.updatedb();
      reply(updateResult);
    }
  } catch (error) {
    console.error('Git update error:', error);
    reply(`❌ *Error while updating!*\nMake sure git is configured correctly.`);
  }
  break;
}
default:
